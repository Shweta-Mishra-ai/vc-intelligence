...
async function scrapeWebsite(url: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      "Failed to scrape the website. It might be protected or invalid."
    );
  }

  try {
    if (!response.ok) {
      throw new Error("Failed to scrape the website. It might be protected or invalid.");
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const errorMessage =
        scrapeError instanceof Error ? scrapeError.message : "Failed to scrape the website. It might be protected or invalid.";
      return NextResponse.json(
        { error: errorMessage },
        { status: 400 }
      );
